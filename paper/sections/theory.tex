...
empirically (Section~\ref{sec:experiments}). The analysis treats one channel of
...
react to a step change. This is exactly the quantity we compute from
each trained layer's mean learned base decay $\bar{a}$
(Eq.~\ref{eq:base-decay}) and compare against the empirically measured
delay between an anomaly's labeled onset and the first score-threshold
crossing.
